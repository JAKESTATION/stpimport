...
def write_named_step(shapes_with_names, filepath):
    """Writes one or more (name, shape) pairs to a single STEP file, with
    each shape's name embedded as a real STEP PRODUCT entity (via an XCAF/
    XDE document), so downstream tools (e.g. a STEP-to-CATPart converter)
    see meaningful part names instead of a generic translator name."""
    ...
    # Same lazy-init behavior verified for the plain STEPControl_Writer
    # applies here too -- set after constructing the writer.
    Interface_Static.SetCVal_s("write.step.unit", "MM")
    writer.Transfer(doc, STEPControl_StepModelType.STEPControl_AsIs)
    writer.Write(filepath)


def write_iges(shape, filepath):
...
def sanitize_filename(name):
    """Makes a Blender object name safe to use as a filename across
    Windows/macOS/Linux -- replaces reserved/problematic characters."""
...
class STEPIMPORT_OT_export(Operator, ExportHelper):
    ...
    individual_files: BoolProperty(
        name="Save as Individual Parts",
        description=(
            "Export each object to its own file (named after the object), "
            "instead of combining every object into one file as multiple "
            "solids. Useful for downstream per-file converters (e.g. a "
            "STEP-to-CATPart pipeline) that expect one part per file"
        ),
        default=True,
    )
    ...
    _current_tris = None
    _triangle_index = 0
    _shapes = None  # list of (object_name, TopoDS shape) pairs, finalized as we go
    ...
    def _step(self, context):
        ...
        if self._phase == 'finalize_object':
            ...
            obj = self._objects[self._object_index]
            self._shapes.append((obj.name, shape))
        ...
        if self._phase == 'pending_write':
            if self.individual_files:
                self._set_status(context, "Writing files...")
            else:
        ...
        if self._phase == 'write':
            ...
            ext = ".iges" if self._file_format == "iges" else ".step"
            ...
            if self.individual_files:
                ...
                for name, shape in self._shapes:
                    ...
                    out_path = os.path.join(out_dir, candidate + ext)

                    if self._file_format == "iges":
                        write_iges(shape, out_path)
                    else:
                        write_named_step([(name, shape)], out_path)
            else:
                if self._file_format == "iges":
                    # IGES has no equivalent widely-supported per-solid naming
                    # mechanism to STEP's PRODUCT entity, so a merged IGES
                    # file combines shapes unnamed, same as before.
                    builder = BRep_Builder()
                    compound = TopoDS_Compound()
                    builder.MakeCompound(compound)
                    for _, shape in self._shapes:
                        builder.Add(compound, shape)
                    write_iges(compound, self.filepath)
                else:
                    write_named_step(self._shapes, self.filepath)
        ...
        if self._phase == 'done':
            ...
            self._cleanup(context)
            if self.individual_files:
                self.report({'INFO'}, f"Exported {count} object(s) to {count} individual {self._file_format.upper()} file(s).")
            else:
                self.report({'INFO'}, f"Exported {count} object(s) to one {self._file_format.upper()} file.")
            return True
