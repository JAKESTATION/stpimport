bl_info = {
    "name": "STEP/IGES Importer (OCP)",
    "author": "Custom",
    "version": (0, 2, 0),
    "blender": (3, 6, 0),
    "location": "File > Import > STEP/IGES (.step/.stp/.iges/.igs)",
    "description": "Import STEP (AP203/AP214/AP242) and IGES CAD files using the OCP OpenCASCADE kernel",
...
def import_cad_file(filepath, linear_deflection_mm, angular_deflection_deg,
                     split_solids, merge_vertices, manual_scale=1.0, file_format="step"):
    import math
    ...
    # OCCT reads whatever length unit the file declares (mm, inch, cm, m...)
    ...
    # OCCT do the correct per-file unit conversion for us automatically. This
    # parameter and behavior is shared by both the STEP and IGES translators.
    # NOTE: this must be set AFTER constructing the reader — OCCT lazily
    # (re)initializes this static parameter to its MM default the first time
    # a reader is constructed in the process, which silently undoes an
    # earlier override if set beforehand.
    Interface_Static.SetCVal_s("xstep.cascade.unit", "M")

    status = reader.ReadFile(filepath)
    if status != IFSelect_RetDone:
        raise RuntimeError(f"Failed to read {file_format.upper()} file (status={status})")
    ...
    if not created:
        raise RuntimeError(f"No triangulated geometry found in this {file_format.upper()} file.")
...
class STEPIMPORT_OT_import(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if lower_path.endswith(".iges") or lower_path.endswith(".igs"):
            file_format = "iges"
        else:
            file_format = "step"

        try:
            objs = import_cad_file(
                self.filepath,
            ...
            self.report({'ERROR'}, f"{file_format.upper()} import failed: {e}")
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, f"Imported {len(objs)} object(s) from {file_format.upper()} file.")
        return {'FINISHED'}
